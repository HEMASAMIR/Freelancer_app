  ...
  Future<void> _togglePublishStatus() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _isPublished ? 'Listing Published ✓' : 'Listing Deactivated',
          ),
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (_) => MultiBlocProvider(
                            ...
                            child: SearchDetails(listing: widget.listing),
                          ),
                        ),
                      );
                    },
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  ...
                  _AmenitiesTab(listing: widget.listing),
                  _CancellationTab(listing: widget.listing),
                ],
              ),
            ),
          ],
...
class _AvailabilityTabState extends State<_AvailabilityTab> {
  ...
  final Map<DateTime, double> _priceOverrides = {};
  bool _saving = false;

  void _toggleDate(DateTime date) {
    final d = DateTime(date.year, date.month, date.day);
    setState(() {
      _blockedDates.contains(d)
            ...
            _blockedDates
              ...
              ..addAll(
                state.unavailableDates
                    .where((a) => a.date != null)
                    .map(
                      (a) => DateTime(a.date!.year, a.date!.month, a.date!.day),
                    ),
              );
            ...
            _priceOverrides
              ...
              ..addEntries(
                state.priceOverrides
                    .where((a) => a.date != null && a.priceOverride != null)
                    .map(
                      (a) => MapEntry(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Manage which dates are available for booking. Click and drag to select date ranges.',
                style: TextStyle(fontSize: 14, color: AppColors.greyText),
              ),
              const SizedBox(height: 24),

              // Blue Alert Box (Best Offer Commission Note)
              Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                  color: const Color(0xFFF0F7FF),
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(color: const Color(0xFFBFE0FF)),
                ),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: const [
                          ...
                          SizedBox(height: 4),
                          Text(
                            'When you request a "Best Offer" and it is approved, an additional commission fee will be applied to bookings made for those specific dates.',
                            style: TextStyle(
                              fontSize: 13,
              ...
              const Text(
                'Click to select • Drag for range • Shift+Click range • Ctrl+Click multiple',
    ...
    child: Row(
      ...
      children: [
        Container(
          ...
          child: IconButton(
            icon: const Icon(Icons.chevron_left, color: AppColors.inkBlack),
            onPressed: () => setState(
        ...
        Container(
          ...
          child: IconButton(
            icon: const Icon(Icons.chevron_right, color: AppColors.inkBlack),
            onPressed: () => setState(
              () => _focusedMonth = DateTime(
                _focusedMonth.year,
                _focusedMonth.month + 1,
  ...
  Widget _calendarGrid() {
    ...
    final cells = <Widget>[
      ...
      ...List.generate(daysInMonth, (i) {
        ...
        final price =
            _priceOverrides[norm] ?? widget.listing.pricePerNight ?? 0.0;
    ...
    return Container(
      ...
      child: GridView.count(
        ...
        crossAxisCount: 7,
        childAspectRatio:
            0.75, // زودنا الطول شوية عشان السعر ميعملش overflow عمودي
        children: cells,
      ),
    );
  }
  ...
  Widget _dot(Color c, String label, {bool hasBorder = false}) => Row(
    mainAxisSize: MainAxisSize.min, // مهمة جداً عشان ميعملش Error جوه الـ Wrap
    children: [
      Container(
        width: 14,
        height: 14,
...
class _PricingTabState extends State<_PricingTab> {
  ...
  void initState() {
    ...
    _adjustments.add(
      _PriceAdj(name: 'Weekend Rate', percentage: 10, enabled: true),
    );
  }
            ...
            children: [
              ...
              ..._adjustments.asMap().entries.map((e) {
                ...
                return _AdjCard(
                  ...
                  onDelete: () => setState(() => _adjustments.removeAt(e.key)),
                );
              }),
              ...
              if (overrides.isNotEmpty) ...[
                const SizedBox(height: 28),
                _sectionHeader(
                  'Price Preview (Base: EGP ${base.toStringAsFixed(0)})',
...
class _SettingsTabState extends State<_SettingsTab> {
  ...
  Future<void> _loadLifestyles() async {
    ...
    result.fold((_) => setState(() => _lifestylesLoading = false), (list) {
      setState(() {
        _allLifestyles = list;
  ...
  Future<void> _loadAmenities() async {
    ...
    selResult.fold((_) => null, (ids) => _selectedAmenities.addAll(ids));
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'translations': {
        'ar': {'title': _titleAr.text, 'description': _descAr.text},
      },
    };
        ...
        children: [
          ...
          _card(
            children: [
              ...
              const SizedBox(height: 12),
              _outlineField(label: 'Google Maps Link', controller: _mapsLink),
            ],
          ),
          const SizedBox(height: 16),
          ...
          _card(
            children: [
              ...
              _cancellationCard(
                title: 'Limited',
                desc:
                    'Full refund 14 days before check-in, 50% refund 7 days before check-in.',
                value: 'limited',
              ),
              const Divider(height: 1, color: AppColors.dividerGrey),
              _cancellationCard(
                title: 'Firm',
                desc:
                    'Full refund on dates 30 days before check-in. 50% refund > 14 days before check-in.',
                value: 'firm',
                isLast: true,
              ),
            ],
          ...
          _lifestylesLoading
              ...
              : Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: _allLifestyles.map((ls) {
...
class _ConditionsTabState extends State<_ConditionsTab> {
  ...
  static const _systemConditions = [
    ...
    _SysCondition(
      ...
      'Guests must ensure all 4 documents are in the space.',
    ),
    _SysCondition(
      'Maximum occupancy',
      'This property may only host the listing max occupancy.',
    ),
    _SysCondition(
      'No parties or events',
      'Parties, events, or large gatherings are not permitted.',
    ),
    ...
    _SysCondition(
      ...
      'All guests only and night rights must be registered in the system.',
    ),
    _SysCondition(
      'Quiet hours',
      'Guests must observe quiet hours from 11PM to 7AM.',
    ),
    _SysCondition(
      'Respect neighbors',
      'Guests must be respectful and keep noise to the acceptable minimum.',
    ),
  ];
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          ..._all
              .where(
                (c) => !_systemConditions.any(
                  (s) => c.name.toLowerCase().contains(
                    s.name.split(' ').first.toLowerCase(),
                  ),
                ),
              )
              .map(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isSelected
              ? AppColors.primaryBurgundy.withValues(alpha: 0.4)
              : AppColors.dividerGrey,
        ),
      ),
      child: Row(
...
class _CancellationTabState extends State<_CancellationTab> {
  ...
  void initState() {
    super.initState();
    _cancellationPolicy =
        (widget.listing.cancellationPolicy?.isNotEmpty == true)
  ...
  Future<void> _save() async {
    ...
    final data = {'cancellation_policy': _cancellationPolicy};
